...
contract('ERC20', function (accounts) {
        ...
        describe('when there was no approved amount before', function () {
          it('reverts', async function () {
            await expectRevert(this.token.decreaseAllowance(
              spender, amount, { from: initialHolder }), 'evm: execution reverted',
            );
          });
        });
  ...
  describe('increase allowance', function () {
    ...
    describe('when the spender is the zero address', function () {
      ...
      it('reverts', async function () {
        await expectRevert(
          this.token.increaseAllowance(spender, amount, { from: initialHolder }), 'evm: execution reverted',
        );
      });
    });
  });
  ...
  describe('_mint', function () {
    ...
    it('rejects a null account', async function () {
      await expectRevert(
        this.token.mint(ZERO_ADDRESS, amount), 'evm: execution reverted',
      );
    });
    ...
    describe('for a non zero account', function () {
      it('rejects burning more than balance', async function () {
        await expectRevert(this.token.burn(
          initialHolder, initialSupply.addn(1)), 'evm: execution reverted',
        );
      });
