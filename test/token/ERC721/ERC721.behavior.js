...
function shouldBehaveLikeERC721 (errorPrefix, owner, newOwner, approved, anotherApproved, operator, other) {
  ...
  context('with minted tokens', function () {
    ...
    describe('balanceOf', function () {
      ...
      context('when querying the zero address', function () {
        it('throws', async function () {
          await expectRevert(
            this.token.balanceOf(ZERO_ADDRESS), 'evm: execution reverted',
          );
        });
      });
    });
    ...
    describe('ownerOf', function () {
      ...
      context('when the given token ID was not tracked by this token', function () {
        ...
        it('reverts', async function () {
          await expectRevert(
            this.token.ownerOf(tokenId), 'evm: execution reverted',
          );
        });
      });
    });
    ...
    describe('approve', function () {
      ...
      context('when the address that receives the approval is the owner', function () {
        it('reverts', async function () {
          await expectRevert(
            this.token.approve(owner, tokenId, { from: owner }), 'evm: execution reverted',
          );
        });
      });
  ...
  describe('_mint(address, uint256)', function () {
    it('reverts with a null destination address', async function () {
      await expectRevert(
        this.token.mint(ZERO_ADDRESS, firstTokenId), 'evm: execution reverted',
      );
    });
    ...
    context('with minted token', async function () {
      ...
      it('reverts when adding a token id that already exists', async function () {
        await expectRevert(this.token.mint(owner, firstTokenId), 'evm: execution reverted');
      });
    });
  });
  ...
  describe('_burn', function () {
    it('reverts when burning a non-existent token id', async function () {
      await expectRevert(
        this.token.burn(nonExistentTokenId), 'evm: execution reverted',
      );
    });
    ...
    context('with minted tokens', function () {
      ...
      context('with burnt token', function () {
        ...
        it('deletes the token', async function () {
          expect(await this.token.balanceOf(owner)).to.be.bignumber.equal('1');
          await expectRevert(
            this.token.ownerOf(firstTokenId), 'evm: execution reverted',
          );
        });

        it('reverts when burning a token id that has been deleted', async function () {
          await expectRevert(
            this.token.burn(firstTokenId), 'evm: execution reverted',
          );
        });
      });
    });
  ...
  context('with minted tokens', function () {
    ...
    describe('tokenOfOwnerByIndex', function () {
      ...
      describe('when the index is greater than or equal to the total tokens owned by the given address', function () {
        it('reverts', async function () {
          await expectRevert(
            this.token.tokenOfOwnerByIndex(owner, 2), 'evm: execution reverted',
          );
        });
      });
      ...
      describe('when the given address does not own any token', function () {
        it('reverts', async function () {
          await expectRevert(
            this.token.tokenOfOwnerByIndex(other, 0), 'evm: execution reverted',
          );
        });
      });
      ...
      describe('after transferring all tokens to another user', function () {
        ...
        it('returns empty collection for original owner', async function () {
          ...
          await expectRevert(
            this.token.tokenOfOwnerByIndex(owner, 0), 'evm: execution reverted',
          );
        });
      });
    });
    ...
    describe('tokenByIndex', function () {
      ...
      it('reverts if index is greater than supply', async function () {
        await expectRevert(
          this.token.tokenByIndex(2), 'evm: execution reverted',
        );
      });
    ...
    context('with minted tokens', function () {
      ...
      context('with burnt token', function () {
        ...
        it('burns all tokens', async function () {
          ...
          await expectRevert(
            this.token.tokenByIndex(0), 'evm: execution reverted',
          );
        });
      });
    });
  ...
  describe('metadata', function () {
    ...
    describe('token URI', function () {
      ...
      it('reverts when queried for non existent token id', async function () {
        await expectRevert(
          this.token.tokenURI(nonExistentTokenId), 'evm: execution reverted',
        );
      });
