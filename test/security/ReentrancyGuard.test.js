...
contract('ReentrancyGuard', function (accounts) {
  ...
  it('does not allow remote callback', async function () {
    ...
    await expectRevert(
      this.reentrancyMock.countAndCall(attacker.address), 'evm: execution reverted');
  });
  ...
  it('does not allow local recursion', async function () {
    await expectRevert(
      this.reentrancyMock.countLocalRecursive(10), 'evm: execution reverted',
    );
  });

  it('does not allow indirect local recursion', async function () {
    await expectRevert(
      this.reentrancyMock.countThisRecursive(10), 'evm: execution reverted',
    );
  });
});
