...
contract('Create2', function (accounts) {
  ...
  describe('deploy', function () {
    ...
    it('fails deploying a contract in an existent address', async function () {
      ...
      await expectRevert(
        this.factory.deploy(0, saltHex, constructorByteCode, { from: deployerAccount }), 'evm: execution reverted',
      );
    });

    it('fails deploying a contract if the bytecode length is zero', async function () {
      await expectRevert(
        this.factory.deploy(0, saltHex, '0x', { from: deployerAccount }), 'evm: execution reverted',
      );
    });
